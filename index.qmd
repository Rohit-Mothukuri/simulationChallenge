...
# Create comparison plot (stacked vertically)
fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(6, 4))

# Plot 1: Modified Strategy
log_bins_mod = np.logspace(np.log10(max(1, np.min(modified_final_balances[modified_final_balances > 0])) * 0.9),
                           np.log10(np.max(modified_final_balances) * 1.1), 25)
n_mod, bins_mod, patches_mod = ax1.hist(modified_final_balances, bins=log_bins_mod, 
                                        edgecolor='black', linewidth=1.2, alpha=0.85, color='steelblue')

ax1.set_xscale('log')
# Set x-axis limits to ensure all reference lines are visible (including initial balance)
# Ensure initial balance is explicitly included in visible range
all_refs_mod = [initial_balance, modified_mean, modified_median]
data_min = np.min(modified_final_balances[modified_final_balances > 0]) if len(modified_final_balances[modified_final_balances > 0]) > 0 else 1
data_max = np.max(modified_final_balances)
xlim_min_mod = max(1, min(data_min, *all_refs_mod) * 0.7)  # Ensure initial balance is visible
xlim_max_mod = max(data_max, *all_refs_mod) * 1.3  # Ensure all reference lines visible
ax1.set_xlim(xlim_min_mod, xlim_max_mod)

ax1.axvline(x=initial_balance, color='red', linestyle='--', linewidth=2.5, 
           label=f'Initial: ${initial_balance/1000:.0f}K', zorder=10, alpha=0.9)
ax1.axvline(x=modified_mean, color='green', linestyle='--', linewidth=2.5, 
           label=f'Mean: ${modified_mean/1000:.0f}K', zorder=10, alpha=0.9)
ax1.axvline(x=modified_median, color='orange', linestyle='--', linewidth=2.5, 
           label=f'Median: ${modified_median/1000:.0f}K', zorder=10, alpha=0.9)
...
ax1.set_title(f'Modified Strategy (50% bet)\nP(> $30K) = {modified_prob_above_30k:.3f} ({modified_prob_above_30k*100:.1f}%)',
             fontsize=13, fontweight='bold')
ax1.grid(True, alpha=0.3, axis='both', linestyle=':', linewidth=0.8)
ax1.legend(fontsize=10, framealpha=0.9)
ax1.tick_params(labelsize=10)

# Plot 2: Original Strategy
log_bins_orig = np.logspace(np.log10(max(1, np.min(original_final_balances[original_final_balances > 0])) * 0.9),
                            np.log10(np.max(original_final_balances) * 1.1), 25)
n_orig, bins_orig, patches_orig = ax2.hist(original_final_balances, bins=log_bins_orig,
                                          edgecolor='black', linewidth=1.2, alpha=0.85, color='steelblue')

ax2.set_xscale('log')
ax2.axvline(x=initial_balance, color='red', linestyle='--', linewidth=2.5,
           label=f'Initial: ${initial_balance/1000:.0f}K', zorder=10, alpha=0.9)
ax2.axvline(x=np.mean(original_final_balances), color='green', linestyle='--', linewidth=2.5,
           label=f'Mean: ${np.mean(original_final_balances)/1000:.0f}K', zorder=10, alpha=0.9)
ax2.axvline(x=np.median(original_final_balances), color='orange', linestyle='--', linewidth=2.5,
           label=f'Median: ${np.median(original_final_balances)/1000:.0f}K', zorder=10, alpha=0.9)

ax2.xaxis.set_major_formatter(plt.FuncFormatter(format_k))
ax2.set_xlabel('Final Balance ($)', fontsize=12, fontweight='bold')
ax2.set_ylabel('Frequency', fontsize=12, fontweight='bold')
ax2.set_title(f'Original Strategy (100% bet)\nP(> $30K) = {original_prob_above_30k:.3f} ({original_prob_above_30k*100:.1f}%)',
             fontsize=13, fontweight='bold')
ax2.grid(True, alpha=0.3, axis='both', linestyle=':', linewidth=0.8)
